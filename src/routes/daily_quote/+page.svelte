<script lang="ts">
  ...
  let soundEnabledLabel = $derived(
    soundEnabled ? 'Desactivar efecto de sonido' : 'Activar efecto de sonido',
  );
  ...
  $effect(() => {
    const cleanup = helpers.registerCommand({
      ...
      description:
        'Activa o desactiva el efecto de escritura de la animacion de la cita.',
